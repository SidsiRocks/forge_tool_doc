...
class Message:
    ...
    def __init__(self, msg_type: MsgType, args_arr: Union[List['Message'],
                                                          Variable]):
...
class Role:
    ...
    def __init__(self, role_name: str, var_dict: Dict[str, Variable],
                 msg_trace: List[Tuple[SendRecv, Message]]):
...
class Protocol:
    ...
    def is_valid_role_name(self, role_name: str):
        return (role_name in map(lambda role_obj: role_obj.role_name,
                                 self.roles_arr))
...
class Strand:
    ...
    def __init__(self, role_name: str, trace_len: int, var_map: Dict[str,
                                                                     str]):
        self.role_name = role_name
...
class Skeleton:
    ...
    def __init__(self, prot_name: str, var_dict: Dict[str, Variable],
                 strand_list: List[Strand], orig_constr: List[Constraint]):
        self.prot_name = prot_name
        self.var_dict = var_dict
...
def s_expr_instead_of_str(expected_str: str, s_expr) -> ParseException:
    ...
    return ParseException(
        f"Expected '{expected_str}' string not an s expression {str(s_expr)} with type {type(s_expr)}"
    )


def unexpected_str_error(expected_str: str,
                         unexpected_str: str) -> ParseException:
    ...
    return ParseException(
        f"Expected '{expected_str}' string at beginning of s expression not {unexpected_str}"
    )
...
def get_var(var_name_str: str, variable_dict: Dict[str, Variable]) -> Variable:
    ...
    if var_name_str not in variable_dict:
        raise ParseException(
            f"Unknown Variable Name {var_name_str} known variables are {variable_dict.keys()}"
        )
...
def match_var_type(var: Variable, expected_var_type: VarType) -> None:
    ...
    if var.var_type != expected_var_type:
        raise ParseException(
            f"Expected var type = {vartype_to_str(expected_var_type)} but got {vartype_to_str(var.var_type)}"
        )
...
def get_int_from_s_expr(s_expr, data_name: str) -> int:
...
#TODO: this function might be uncessary commenting out for now will
#remove later
"""
...
def parse_ltk(s_expr, var_dict: Dict[str, Variable]) -> Message:
    ...
    if len(s_expr) != 3:
        raise ParseException(
            f"Expected s-expr of length 3 ltk name1 name2 but length is {len(s_expr)}"
        )
    ...
    if name_1.var_type != VarType.NAME:
        raise ParseException(
            f"Expected variable type {vartype_to_str(VarType.NAME)} got {vartype_to_str(name_1.var_type)}"
        )
    if name_2.var_type != VarType.NAME:
        raise ParseException(
            f"Expected variable type {vartype_to_str(VarType.NAME)} got {vartype_to_str(name_2.var_type)}"
        )
...
def parse_pubk(s_expr, var_dict: Dict[str, Variable]) -> Message:
    ...
    if len(s_expr) != 2:
        raise ParseException(
            f"Expected s-expr of length 2 pibk name but length is {len(s_expr)} s_expr is {str(s_expr)}"
        )
    ...
    if name.var_type != VarType.NAME:
        raise ParseException(
            f"Expected variable type {vartype_to_str(VarType.NAME)} got {vartype_to_str(name.var_type)}"
        )
...
def parse_privk(s_expr, var_dict: Dict[str, Variable]) -> Message:
    ...
    if len(s_expr) != 2:
        raise ParseException(
            f"Expected s-expr of length 2 privk, name but length is {len(s_expr)} s_expr is {str(s_expr)}"
        )
    ...
    if name.var_type != VarType.NAME:
        raise ParseException(
            f"Expected varaible type {vartype_to_str(VarType.NAME)} got {vartype_to_str(name.var_type)}"
        )
    name_term = Message(MsgType.ATOM_TERM, name)
...
def is_valid_key(msg: Message) -> bool:
    ...
    if msg.msg_type in [MsgType.PUBK_TERM, MsgType.LTK_TERM]:
        return True
    if msg.msg_type == MsgType.ATOM_TERM and msg.msg_data.var_type in [
            VarType.AKEY, VarType.SKEY
    ]:
        return True
    return False
...
def parse_enc_term(s_expr, var_dict: Dict[str, Variable]) -> Message:
    ...
    if len(s_expr) < 3:
        raise ParseException(
            f"Expected atleast three terms enc,message and encryption key")
    ...
    if not is_valid_key(key_term):
        raise ParseException(
            f"Expected term of type key but got {str(s_expr)}")
...
def parse_atom(s_expr: sexpdata.Symbol, var_dict: Dict[str,
                                                       Variable]) -> Variable:
...
def parse_term(s_expr, var_dict: Dict[str, Variable]) -> Message:
    ...
    if len(s_expr) < 2:
        raise ParseException(
            f"Since s-expr in message expected enc or cat followed by message term"
        )
    ...
    if enc_cat_ltk_pubk_str not in func_dict:
        raise ParseException(
            f"Expected s-expr message term to start with enc/cat/pubk/ltk not {enc_cat_ltk_pubk_str}"
        )
...
def parse_protocol(s_expr) -> Protocol:
    ...
    return Protocol([parse_role(role_expr) for role_expr in s_expr[3:]],
                    prot_name, BASIC_STR)
...
def parse_var_mapping(s_expr, var_map_dict: Dict[str, str], role_obj: Role):
    ...
    if len(s_expr) != 2:
        raise ParseException(
            "Expected skeleton variable name,role variable name")
    ...
    if not role_obj.is_valid_var_name(role_var_name):
        raise ParseException(
            f"Varible name '{role_var_name}' and type '{type(role_var_name)}' not present in role declaration '{role_obj.role_name}' which has variable list '{list(role_obj.var_dict.keys())}' '{list(map(type,list(role_obj.var_dict.keys())))}'"
        )
    if (skelet_var_name in var_map_dict):
        raise ParseException(
            f"Repeated variable name {skelet_var_name} in defstrand clause")
...
def parse_strand(s_expr, prot_obj: Protocol) -> Strand:
    ...
    if len(s_expr) < 4:
        raise ParseException(
            "Expected defstrand,role_name,strand len?,variable mappings")
    ...
    if not prot_obj.is_valid_role_name(role_name):
        raise ParseException(
            f"Role Name {role_name} has not been declared in defprotocol clause"
        )
...
def parse_non_orig(s_expr, var_dict: Dict[str, Variable],
                   constr_lst: List[Constraint]):
...
def parse_uniq_orig(s_expr, var_dict: Dict[str, Variable],
