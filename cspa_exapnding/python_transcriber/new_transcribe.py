...
class RoleTranscribeContext:
    ...
    def get_variable_name(self,var_name:str):
        if var_name not in self.role.var_map:
            raise ParseException(f"cannot get var name for {var_name} not in {self.role.var_map}")
        return f"{self.role_var_name}.{self.role_sig_name}_{var_name}"
def transcribe_role_to_sig(role:Role,role_sig_name:str,transcr:Transcribe_obj):
    field_name_type = [(var_name,f"one {vartype_to_str(var.var_type)}") for var_name,var in role.var_map.items()]
    transcr.write_sig(role_sig_name,field_name_type)
def transcribe_var(elm_expr:str,var:Variable,role_context:RoleTranscribeContext):
    role_context.transcr.print_to_file(f"{elm_expr} = {role_context.get_variable_name(var.var_name)}")
def transcribe_enc(elm_expr:str,enc_term:EncTerm,role_context:RoleTranscribeContext):
    ...
    data_atom_names = [f"atom_{transcr.get_fresh_num()}" for _ in range(len(enc_term.data))]
    ...
    for indx,data_atom_name in enumerate(data_atom_names):
        transcibe_non_cat(data_atom_name,enc_term.data[indx],role_context)
...
def transcribe_ltk(elm_expr:str,ltk_term:LtkTerm,role_context:RoleTranscribeContext):
    role_context.transcr.print_to_file(f"{elm_expr} = getLTK[{ltk_term.agent1_name},{ltk_term.agent2_name}]")
def transcribe_pubk(elm_expr:str,pubk_term:PubkTerm,role_context:RoleTranscribeContext):
    role_context.transcr.print_to_file(f"{elm_expr} = getPUBK[{pubk_term.agent_name}]")
def transcribe_privk(elm_expr:str,privk_term:PrivkTerm,role_context:RoleTranscribeContext):
    role_context.transcr.print_to_file(f"{elm_expr} = getPRIVK[{privk_term.agent_name}]")
def transcribe_non_cat(elm_expr:str,msg:NonCatTerm,role_context:RoleTranscribeContext):
    match msg:
...
def transcribe_indv_trace(role:Role,indx:int,role_context:RoleTranscribeContext):
...
def transcribe_trace(role:Role,role_var_name:str,role_context:RoleTranscribeContext):
    ...
    for i,(send_recv,trace) in enumerate(role.trace):
