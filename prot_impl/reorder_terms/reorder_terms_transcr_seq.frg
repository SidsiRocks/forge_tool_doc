...
sig reorder_terms_A extends strand{
    reorder_terms_A_a: one name,
    reorder_terms_A_b: one name,
    reorder_terms_A_n1: one text,
    reorder_terms_A_n2: one text
}

// predicate follows below
pred exec_reorder_terms_A {
    all arbitrary_reorder_terms_A : reorder_terms_A | {
        some t0,t1 : Timeslot | {
            t1 in t0.(^next)
            
            t0 + t1  = sender.arbitrary_reorder_terms_A + receiver.arbitrary_reorder_terms_A
            
            t0.sender = arbitrary_reorder_terms_A
            t1.receiver = arbitrary_reorder_terms_A
            
            inds[t0.data] = 0 + 1
            some atom1,atom2 : elems[t0.data] {
                (t0.data)[0] = atom1
                (t0.data)[1] = atom2
                atom1 = arbitrary_reorder_terms_A.reorder_terms_A_n1
                atom2 = arbitrary_reorder_terms_A.reorder_terms_A_n2
            }
            inds[t1.data] = 0 + 1
            some atom3,atom4 : elems[t1.data] {
                (t1.data)[0] = atom3
                (t1.data)[1] = atom4
                atom3 = arbitrary_reorder_terms_A.reorder_terms_A_n2
                atom4 = arbitrary_reorder_terms_A.reorder_terms_A_n1
            }
        }
    }
}
...
sig reorder_terms_B extends strand{
    reorder_terms_B_n1: one text,
    reorder_terms_B_n2: one text,
    reorder_terms_B_a: one name,
    reorder_terms_B_b: one name
}

// predicate follows below
pred exec_reorder_terms_B {
    all arbitrary_reorder_terms_B : reorder_terms_B | {
        some t0,t1 : Timeslot | {
            t1 in t0.(^next)
            
            t0 + t1  = sender.arbitrary_reorder_terms_B + receiver.arbitrary_reorder_terms_B
            
            t0.receiver = arbitrary_reorder_terms_B
            t1.sender = arbitrary_reorder_terms_B
            
            inds[t0.data] = 0 + 1
            some atom5,atom6 : elems[t0.data] {
                (t0.data)[0] = atom5
                (t0.data)[1] = atom6
                atom5 = arbitrary_reorder_terms_B.reorder_terms_B_n2
                atom6 = arbitrary_reorder_terms_B.reorder_terms_B_n1
            }
            inds[t1.data] = 0 + 1
            some atom7,atom8 : elems[t1.data] {
                (t1.data)[0] = atom7
                (t1.data)[1] = atom8
                atom7 = arbitrary_reorder_terms_B.reorder_terms_B_n2
                atom8 = arbitrary_reorder_terms_B.reorder_terms_B_n1
            }
        }
    }
}
