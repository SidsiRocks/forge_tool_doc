...
(defprotocol woo_and_lam_pi1 basic 
    (defrole init 
        (vars (a b s name) (Nb text))
        (trace 
            (send a)
            (recv Nb)
            (send (enc a b Nb (ltk a s)))
        )
        (constraint 
            (non-orig (ltk a s))
    ...
    (defrole resp 
        ...
        (trace 
            ...
            (send Nb)
            (recv msg)
            (send (enc a b msg (ltk b s)))
            (recv (enc a b Nb (ltk b s)))
        )
        (constraint 
            (non-orig (ltk b s))
    ...
    (defrole server 
        (vars (a b s name) (Nb text))
        (trace 
            (recv (enc a b (enc a b Nb (ltk a s)) (ltk b s)))
            (send (enc a b Nb (ltk b s)))
        )
        (constraint 
            (non-orig (ltk a s))
            (non-orig (ltk b s))
