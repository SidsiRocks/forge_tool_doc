...
Now that we have created our CPSA and forge files and placed them in a directory inside the prot_impl directory we do the following to run it
#+BEGIN_SRC shell
  python3 run_protocol_tuple.py {protocol_directory}
#+END_SRC
Above command generates a forge file of the form protocol_name_tuple_new_transcr.frg
To run the forge file you have to run the below command.
#+BEGIN_SRC shell
  racket ./protocol_directory/protocol_name_tuple_new_transcr.frg
#+END_SRC
This opens up forge's UI for generating and viewing instances in your browser.
