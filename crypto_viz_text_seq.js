...
// TODO in plaintext, contents, data and other relations of this kind
// making the assumption that the components of the sequnce are seen in linear
// order hence the index is not being explicitly checked here, this assumption
// holds true currently but have to change this in the future.
...
function convert_seq_msg(data_tpls){
    var len = data_tpls.length;
...
function parseTerms(items) {
    ...
    const seqStrings = instance.signature('seq').atoms().map(e => e.toString())
    ...
    const newItems = items.map((item) => {
        ...
        if (pubKeyMap[itemString]) {
            ...
            return {
                content : parseTerms(contents_of_seq),
                isSeq : true
            }
        } else {
            return {
...
function flattenParsedTerms(parsedTerms) {
    ...
    for (i = 0; i < parsedTerms.length; i++) {
        ...
        if (term["subscript"]) {
            ...
            array.push({content:"["});
            //array = array.concat(flattenParsedTerms(term.content))
            let contents_array = flattenParsedTerms(term.content)
            contents_array.forEach((content) => {
                array.push(content);
                array.push({content:' '});
            })
            array.push({content:"]"});
...
function labelText(m) {
    ...
    const parsed = parseTerms(convert_seq_msg(data));
